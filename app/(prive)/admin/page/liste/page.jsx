...
export default function ListeOffres() {
    ...
    useEffect(() => {
        ...
        const fetchOffres = async () => {
            try {
                ...
                const response = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/page`);
    ...
    return (
        <div className="px-4 py-8 dark:text-gray-200 dark:bg-gray-700 w-full h-screen">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Gestion des pages</h1>
                <Link
                    href={`/admin/page/add`}
    ...
    async function handleDelete(id) {
        if (confirm('Êtes-vous sûr de vouloir supprimer cette élément ?')) {
            try {
                const response = await axios.delete(`/api/page?id=${id}`);

                if (!response.statusText) {
                    throw new Error('Échec de la suppression');
                }

                // Mettre à jour la liste des pages après suppression
                setPages(pages.filter(elt => elt.id !== id));
            } catch (err) {
                setError(err.message);
            }
        }
