import { safeFetch } from '@/components/composants';
import React from 'react'

export const dynamic = 'force-dynamic';

export async function generateMetadata({ params }) {
    const { slug } = await params
    const [article] = await safeFetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/articles?meta=df&slug=${slug}`)

    return {
        title: article?.metaTitle || 'Les 3 Merveilles',
        description: article?.metaDescription || 'Découvrez notre sélection exclusive de chocolats fins et produits d\'affiliation de qualité',
        robots: article?.status == true ? 'index, follow' : 'noindex, nofollow',
        alternates: {
            canonical: `${process.env.NEXT_PUBLIC_SITE_URL}/blog/${slug}`,
        },
        openGraph: {
            title: article?.meta_title || 'Les 3 Merveilles',
            description: article?.meta_description || 'Découvrez notre sélection exclusive de chocolats, technologie et la mode d\'aujourd\'hui d\'affiliation de qualité',
            url: `${process.env.NEXT_PUBLIC_SITE_URL}/blog/${slug}`,
            // Fiche produit:	product
            // Page catégorie:	website
            // Page article (blog):	article
            // Page d’accueil:	website
            type: 'article',
            siteName: 'Les 3 Merveilles',
            images: [{
                url: article?.image,
                width: 1200,
                height: 630,
                alt: article?.meta_title || 'les 3 merveilles'
            }],
            other: {
                'fb:app_id': '978066750965088',
            },
        },
    };
}

const page = async ({ params }) => {
    const { slug } = await params
    const [article] = await safeFetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/articles?slug=${slug}`)

    return (
        <div>
            <section
                className="relative bg-cover bg-center h-auto dark:bg-gray-800 p-10 md:p-0 md:h-[500px]"
                style={{ backgroundImage: `url('${article?.image}')` }}
            >
                {/* Overlay pour améliorer la lisibilité */}
                <div className="absolute inset-0 bg-black opacity-50"></div>
                <div className="relative z-10 flex flex-col justify-center items-center h-full text-center px-4">
                    <h1 className="text-4xl md:text-6xl font-bold text-white">
                        {article?.title}
                    </h1>
                    <h2 className="mt-4 text-lg md:text-xl text-gray-300 lg:w-[1000px]">
                        {article?.description}
                    </h2>
                </div>
            </section>
            <section className='bg-gray-50 dark:bg-gray-900 dark:text-gray-200 py-20'>
                {article?.content ? (
                    <div className="overflow-x-auto max-w-none sm:mx-[100px] mx-5">
                        <div className="no-tailwind" dangerouslySetInnerHTML={{ __html: JSON.parse(article.content) }} />
                    </div>
                ) : (
                    <p>Contenu indisponible.</p>
                )}
            </section>
        </div>
    )
}

export default page