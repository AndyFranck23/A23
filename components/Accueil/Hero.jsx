"use client"

import Image from "next/image";
import { scrollToSection } from "../composants";

// components/Hero.jsx
export default function Hero() {

    return (
        <section className="relative h-auto md:h-[500px] p-10 md:p-0 overflow-hidden">
            {/* Image de fond */}
            <Image
                src="/hero.webp"
                alt="Visuel des 3 Merveilles"
                fill
                quality={20}
                className="object-cover object-center"
                priority
            />

            {/* Overlay pour le contraste */}
            <div className="absolute inset-0 bg-black opacity-50" />

            {/* Contenu au-dessus */}
            <div className="relative z-10 flex flex-col justify-center items-center h-full text-center px-4">
                <h1 className="text-4xl md:text-6xl font-bold text-white">
                    Bienvenue sur Les 3 Merveilles
                </h1>
                <h2 className="mt-4 text-lg md:text-xl text-gray-300 lg:w-[1000px]">
                    Les 3 Merveilles : chocolats artisanaux, technologie alliant performance et design, mode tendance pour sublimer chaque look.
                </h2>
                <button
                    onClick={() => scrollToSection("offreSection")}
                    className="mt-6 px-6 py-3 bg-tech hover:bg-amber-200 hover:text-gray-900 text-white rounded-full font-semibold transition"
                >
                    Explorer
                </button>
            </div>
        </section>
    );
}
